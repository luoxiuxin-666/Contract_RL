...
# 导入自定义模块
from UsualFunctions import LOG, CommonFun  # 假设这是您的日志工具
...
class SFLExecutionRunner:
    def __init__(self, config):
        ...
        self.agent = PPO(
            ...
            n_dn=config.N_DN,
            n_cn=config.M_CN,
            lr_actor_cont=config.LR_ACTOR[0],  # 连续动作学习率 (大)
            lr_actor_disc=config.LR_ACTOR[1],  # 离散动作学习率 (小，防止震荡)
    ...
    def run_training(self):
        ...
        for i_episode in range(1, self.cfg.TOTAL_EPISODES + 1):
            ...
            while True:
                ...
                proc_cont, disc_action = self.agent.select_action(state)

                # 2.2 环境交互
                # 环境内部会进行 IC/IR 检查、能耗计算、奖励计算
                next_state, reward, done, total_time,dn_contract,cn_contract,compliance_rate = self.env.step(proc_cont, disc_action)

                W_all = np.round(proc_cont[self.env.N_DN + self.env.M_CN:],2) * (self.env.TOTAL_BW/1e6)
                ...
                episode_time.append(total_time)  # 假设 ep_time 是本轮总耗时
            ...
            if i_episode % self.cfg.LOG_INTERVAL == 0:
                ...
                log_msg = f"Ep {i_episode} | Reward: {avg_reward:.2f} | LR_Cont: {lrs.get('cont', 0):.2e}|avg_time: {avg_time:.2f}"
                Log(log_msg)
